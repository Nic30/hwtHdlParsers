...
def mkFileInfo(*fileNames, lib="work"):
    fis = []
    for f in fileNames:
        fi = ParserFileInfo(VHDL_DIR + f, lib)
        fis.append(fi)
    return fis
...
class ParserTC(unittest.TestCase):
    ...
    def testVerilogSimpleMuxModule(self):
        fi = ParserFileInfo(VERILOG_DIR + "mux.v", None)
    ...
    def testVerilogModuleParams(self):
        fi = ParserFileInfo(VERILOG_DIR + "simpleParam.v", None)
        fi.primaryUnitsOnly = True
    ...
    def testVerilogParamSpecifiedByParam(self):
        fi = ParserFileInfo(VERILOG_DIR + "simpleParam2.v", None)
        fi.primaryUnitsOnly = True        
